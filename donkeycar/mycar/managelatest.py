...
from donkeycar.parts.Controll import Controll
from donkeycar.parts.stop import Stop
from donkeycar.parts.resizer import Resizer
...
from donkeycar.parts.sign_receive import Sign_receiver
from donkeycar.parts.roadsign_detector import Roadsign_detector
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    #camResizer
    resizer=Resizer()
    V.add(resizer,inputs=['camHigh/image_array'],outputs=['cam/image_array'])

    #This web controlcontroller will create a web server that is capable
    #of managing steering, throttle, and modes, and more.
    ctr = LocalWebController(port=cfg.WEB_CONTROL_PORT, mode=cfg.WEB_INIT_MODE)

    V.add(ctr,
        inputs=['camHigh/image_array', 'tub/num_records'],
    ...
    V.add(DriveMode(),
          ...
          outputs=['angle', 'throttle_temp'])

    #Sensor
    if cfg.SIGN_DETECT:
        Sign_rec = Sign_receiver(pi, cfg.SIGN_GPIOS)
        V.add(Sign_rec, outputs=['left','right','stop'])
    elif cfg.LOCAL_SIGN_DETECTOR:
        local_detector=Roadsign_detector()
        V.add(local_detector,inputs=['camHigh/image_array'],outputs=['left','right','stop'])

    if cfg.RANGE_ENABLE:
        Range = Sensor(pi, cfg.RANGE_GPIOS)
        V.add(Range, outputs=['range/cms'], threaded=False)

    #stop when too close to front
    V.add(Stop(),inputs=['throttle_temp','range/cms','stop'],outputs=['throttle_1'])

    V.add(Controll(), inputs=['throttle_1'], outputs=['throttle'], threaded=True)
    ...
    if cfg.RANGE_ENABLE and False:#Enable Range Data into Tub
        inputs += ['range/cms']
        types += ['float']
